...
def _process_win_prob(
    win_prob: float,
    return_buckets_edges: torch.tensor,
) -> torch.tensor:
    pass


class ChessDataset(Dataset):
    def __init__(self, bag_path, policy='action_value', num_return_bucket=128):
        self.bag_reader = BagReader(bag_path)
        self.policy = policy
        self.num_return_bucket = num_return_bucket
    ...
    def __getitem__(self, idx):
        ...
        if self.policy == 'action_value':
            ...
            return_bucket = _process_win_prob(
                win_prob, self.return_buckets_edges)
